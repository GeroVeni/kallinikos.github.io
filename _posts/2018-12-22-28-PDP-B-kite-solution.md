---
title: "28ος ΠΔΠ - Β' φάση Γυμνάσιο - Λύση"
layout: post
tags: [ΠΔΠ]
category: 1
comments: true
...
## Λύση πολυπλοκότητας $O(N^3)$, χώρου $O(N)$
...
int main () {
    ...
    scanf("%ld %ld", &N, &K);
    for (long i = 1; i <= N; ++i) { scanf("%ld", &pieces[i]); }
    
    // Αρχική μέγστιστη τιμή: Ν + 1
    ...
    for (long i = 1; i <= N; ++i) {
        for (long j = i; j <= N; ++j) {
            ...
            long sum = 0;
            for (long k = i; k <= j; ++k) { sum += pieces[k]; }
...
Μία βελτίωση που μπορεί κάποιος να παρατηρήσει σε αυτή την υλοποίηση είναι αντί η επανάληψη μας να γίνεται με βάση το $$i$$ και μετά το $$j$$, να γίνεται πρώτα με βάση το μήκος $$l = i - j + 1$$, $$1 \le l \le N$$ και μετά με βάση το $$i$$. Έτσι, δεν χρειάζεται να κρατάμε τη μεταβλητή `minPieces` γιατί αν βρούμε μία λύση κατά τη διάρκεια αυτής της επανάληψης τότε θα ξέρουμε ότι είναι η βέλτιστη (γιατί έχουμε εξαντλήσει όλες τις περιπτώσεις με μικρότερο μήκος). Επίσης με το που θα βρεθεί η λύση, θα μπορούμε να τερματίσουμε το πρόγραμμα, γλιτώνοντας επαναλήψεις.

## Λύση πολυπλοκότητας $O(N^2)$, χώρου $O(N)$
...
Ας γράψουμε το άθροισμα του διαστήματος από το $$i$$ ως το $$j$$, ως

$$
S(i, j) = \sum_{k=i}^{j}M_k=M_i + M_{i+1} + \dotsb + M_{j-1} + M_{j} = \\
= (M_1 + M_2 + \dotsb + M_{j-1} + M_j) - (M_1 + M_2 + \dotsb +M_{i-2}+M_{i-1}) = \\ =S(1,j) - S(1, i-1) = C(j) - C(i-1)
$$
όπου $$C(i) = M_1 + M_2 + \dotsb + M_{i-1}+M_{i}$$, είναι το άθροισμα από το $$1$$ ως το $$i$$. Χρειάζεται να προϋπολογίσουμε το $$C(i)$$ στην αρχή του προγράμματος, αλλά είναι γρήγορο και δεν πιάνει πολύ χώρο ($$O(N)$$ χρόνο και $$O(N)$$ χώρο). 

Αφού το $$C(i)$$ κρατάει αθροίσματα, θα ονομάσουμε τον πίνακα που θα το αποθηκεύει `sums`

```c++
// ...
    // Αφού διαβάσουμε τα μήκη στον πίνακα pieces μπορούμε 
    // να υπολογίσουμε τον πίνακα sum
    long sums[N + 1];
    sums[0] = 0; // Δεν περιέχει κανένα κομμάτι άρα έχει τιμή 0
    for (long i = 1; i <= N; ++i) {
        // Το sums[i] είναι όσο το sums[i-1] συν
        // το επιπλέον κομμάτι που περιέχει pieces[i]
        sums[i] = sums[i-1] + pieces[i];
    }
// ...
```
...
## Λύση πολυπλοκότητας $O(N\log N)$, χώρου $O(N)$

Η συνάρτηση $$C(i)$$ παρατηρούμε ότι είναι *αύξουσα*, δηλαδή όσο μεγαλώνει το $$i$$, μεγαλώνει και το $$C(i)$$. Αυτό σημαίνει ότι το $$S(i, j) = C(j) - C(i-1)$$, είναι *αύξουσα* ως προς $$j$$ και *φθίνουσα* ως προς $$i$$.
...
Αυτή η ιδιότητα μας δείχνει ότι μόνο μια φορά μπορεί να εμφανίζεται η τιμή $$K$$ σε κάθε γραμμή ή στήλη. Επομένως μπορούμε για κάθε κάθε γραμμή[^1] να εφαρμόσουμε μια δυαδική αναζήτηση (binary search) και να βρούμε την μοναδική εμφάνιση του $$K$$, αν αυτό υπάρχει στη συγκεκριμένη γραμμή.
...
Η συνολική πολυπλοκότητα είναι $$O(N\log N)$$ γιατί εκτελούμε δυαδική αναζήτηση σε κάθε γραμμή.

## Λύση πολυπλοκότητας $O(N)$, χώρου $O(N)$
...
Τότε αν το $$S(I, J) > K$$, τότε συμπεραίνουμε ότι ούτε για $$i = I$$ υπάρχει λύση, καθώς αυτό θα προϋπόθετε το $$j < J$$. Άρα $$i \ge I + 1$$. Επομένως αφού μόλις απορρίψαμε το $$i = I$$, η αρχική υπόθεση θα ισχύει για το ζεύγος $$(I + 1, J)$$.
...
- Αν $$S(I, J) = K$$, τότε αυξάνουμε και το $$I$$ και το $$J$$ κατά $$1$$, και ελέγχουμε αν το διάστημα $$(I, J)$$ έχει μικρότερο μήκος από το έως τώρα ελάχιστο.

Αρχικές τιμές των $$I$$ και $$J$$, θα είναι $$I = J = 1$$, αφού στην αρχή δεν έχουμε ελέγξει κανένα ζευγάρι τιμών.

Ας σημειωθεί ότι η λύση δεν χρειάζεται να κρατάει τον πίνακα με τα μήκη των σχοινιών, αφού δεν χρειάζεται πουθενά πέρα από τον υπολογισμό των αθροισμάτων `sums`.
...
int main () {
    ...
    while(j <= N) {
        ...
        if (S(i, j) == K) {
            ...
            if (j - i + 1 < minPieces) { minPieces = j - i + 1; }
            // Αυξάνουμε το i, αφού δεν πρόκειτε να μας δώσει πάνω
            // από μία λύση
            ++i;
        }
